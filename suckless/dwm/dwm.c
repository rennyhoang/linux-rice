...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		sw = m->ww - drawstatusbar(m, bh, stext) - stw - 2 * sp;
	}
	...
	if ((w = m->ww - sw - stw - x) > bh) {
		if (m->sel) {
			...
			drw_text(drw, x, 0, w - 2 * sp, bh, pad + (m->sel->icon ? m->sel->icw + ICONSPACING : 0), m->sel->name, 0);
			if (m->sel->icon) drw_pic(drw, x + pad, (bh - m->sel->ich) / 2, m->sel->icw, m->sel->ich, m->sel->icon);
			if (m->sel->isfloating)
